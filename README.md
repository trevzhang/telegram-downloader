...
Bot 私聊里只维护**一条**实时消息「📊 tgdl 看板」，内容包括：当前任务进度（进度条、速度、预计剩余、正在下载的文件、限流提示）、排队中的任务、上一个任务的结果。全局只有一个刷新循环：每 `PROGRESS_INTERVAL` 秒重新渲染一次，内容有变化才编辑，任务状态变化时立即刷新；空闲时不产生任何请求。

- `/status`、`/tasks` 不会新发进度消息，而是把看板**移到聊天底部**（删除旧看板、重发一条），始终只有一条。
- 看板底部有内联按钮：「🔄 刷新」立即重绘；「🚫 取消当前」取消正在执行的任务（仅在有任务时显示）。点按钮只原地编辑看板，不产生新消息。
- 每个任务结束时（完成、取消、失败）Bot 另发**一条静态汇总**（成功/跳过/失败清单）作为历史记录，发出后不再编辑。
- 任务开始时不再单独发送概览消息，扫描与下载状态都体现在看板上。
