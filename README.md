...
启动成功后 Bot 会给你发一条「✅ tgdl 已启动，发送 /help 查看用法」；如果你还没给 Bot 发过 `/start`，这条通知会发送失败，程序只记录警告并继续运行。日志同时输出到控制台与 `data/logs/tgdl.log`（按天轮转，保留 14 天）。按 `Ctrl+C` 退出：第一次会优雅关停（取消当前任务、更新概览消息、断开连接，每步最多等待十几秒），如果网络卡住导致迟迟不退出，再按一次 `Ctrl+C` 会强制退出。
