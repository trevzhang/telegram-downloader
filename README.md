...
全局只有一个刷新循环：每 `PROGRESS_INTERVAL` 秒重新渲染一次，内容有变化才编辑，任务状态变化时立即刷新；空闲时不产生任何请求。`/status`、`/tasks`、`/dl`、`/cancel` 都只是原地刷新看板，不会新发进度消息；只有看板不存在（首次启动或被你删掉）时才会新发一条。看板被后续消息顶到上方时，在 Telegram 里把它**置顶**即可随时点开。
