...
**架构一句话：** 单进程 asyncio，两个 Telethon 客户端——**用户账号**负责解析链接、扫描消息、下载媒体；**Bot** 负责接收 OWNER 的命令，并维护一条自动刷新的「看板」消息展示进度。任务串行排队，任务内文件按信号量并发下载。
...
| `PROGRESS_INTERVAL` | 否 | 看板自动刷新间隔（秒，≥1），默认 5；内容没变化时不会编辑 |
...
Bot 私聊里只维护**一条**实时消息「📊 tgdl 看板」，**永远原地编辑，不删除重发**。底部两行内联按钮：

- 第一行切换视图：「📥 进度」当前任务的进度条、速度、预计剩余、正在下载的文件、限流提示；「📋 队列」所有排队与进行中的任务；「📜 历史」最近 5 个结束任务的结果。当前视图前有「•」标记，点按钮只切换内容。
- 第二行操作：「🔄 刷新」立即重绘；「🚫 取消当前」取消正在执行的任务（仅有任务时显示）。

全局只有一个刷新循环：每 `PROGRESS_INTERVAL` 秒重新渲染一次，内容有变化才编辑，任务状态变化时立即刷新；空闲时不产生任何请求。每条文字命令（`/tasks`、`/dl`、`/cancel`）处理完后会把看板**移到聊天最底部**（删除旧看板、重发一条，始终只有一条）；点按钮和自动刷新都是原地编辑，不产生新消息。

每个任务结束时（完成、取消、失败）Bot 另发**一条静态汇总**（成功/跳过/失败清单）作为历史记录，发出后不再编辑。

## 文件存放结构

```text
downloads/<频道名称>/<YYYY_MM>/<消息ID> - <文件名>
```

- `<频道名称>`：优先用频道/群组的显示名称（与你手工整理的目录一致），没有名称时用 username，最后是数字 ID；路径非法字符替换为 `_`。
- `<YYYY_MM>`：按消息发布月份分目录，如 `2026_09`。
- `<消息ID> - <文件名>`：与 [telegram_media_downloader](https://github.com/tangyoha/telegram_media_downloader) 的默认命名一致，同一频道内不会重名；Telegram 上没有文件名的媒体（原生图片、未命名视频）命名为 `<消息ID>.<扩展名>`，如 `159.jpg`。
- 下载过程中写入 `<文件名>.part`，完成后原子重命名为最终文件名。中断后 `.part` 保留，下次从已下载位置**断点续传**（见下）。
...
**性能提示**：文件先写成 `.part` 再在同一目录内重命名，SMB 上这一步是原子的。

## 行为说明

- **跳过已存在文件**：目标文件已存在且大小一致时直接标记「跳过」，不重复下载，因此可以放心重复执行同一命令来补齐。
- **断点续传**：下载按 512 KB 分块追加写入 `.part`。网络错误重试、限流等待、任务取消、程序退出都会保留 `.part`；下次下载同一条消息时从已有长度（对齐到 512 KB）继续，不再从头下。`.part` 比目标文件还大视为损坏，从头重下；恰好完整则直接重命名。只有永久性错误（消息已删除、文件 ID 无效等）才删除 `.part`。
